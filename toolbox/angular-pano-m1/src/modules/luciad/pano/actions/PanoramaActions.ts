...
import {createFadePanoAnimationKey, FadePanoramaAnimation} from "../animation/FadePanoramaAnimation.js";
import {PanoramaFeaturePainter} from "../../painters/PanoramaFeaturePainter.js";
  ...
  leavePanoramaMode(): void {
    ...
    if (this._map.camera instanceof PerspectiveCamera) {
      ...
      if (this._map.reference.referenceType === ReferenceType.GEOCENTRIC) {
        ...
        const eyeLLH = toLLH.transform(this._map.camera.eyePoint);
        // move camera up and back a bit
        eyeLLH.translate3D(0, 0, this._leaveOptions.upDistance);
        const geodesy = createEllipsoidalGeodesy(getReference("CRS:84"))
        targetPoint = geodesy.interpolate(eyeLLH, this._leaveOptions.backDistance, lookFrom.yaw + 180);
      }

      if (this._map.reference.referenceType === ReferenceType.CARTESIAN) {
        targetPoint = this._map.camera.eyePoint.copy();
        const geodesy = createCartesianGeodesy(this._map.reference);
        targetPoint?.translate3D(0, 0, this._leaveOptions.upDistance);
        targetPoint = geodesy.interpolate(targetPoint, this._leaveOptions.backDistance, lookFrom.yaw + 180);
      }
