...
export class FeatureClickController extends PickController {
  ...
  override getPaintRepresentations(event: GestureEvent): PaintRepresentation[] {
    return [PaintRepresentation.BODY];
  }

  override getCandidates(viewPoint: Point | null, sensitivity: number, paintRepresentations: PaintRepresentation[], multiple: boolean): PickInfo[] {
  ...
  override handleCandidates(gestureEvent: GestureEvent, candidates: PickInfo[]): HandleEventResult {
  ...
  override isPickEvent(gestureEvent: GestureEvent): boolean {
  ...
  override  on(event: typeof FEATURE_CLICKED | "Invalidated" | "Activated" | "Deactivated", callback: (...args: any[]) => void,
     context?: any): Handle {
