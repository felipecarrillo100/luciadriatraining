...
export function createPentagon(sensorLocation: Point) {
  const points: Point[] = [];
...
export function createPentagon2D(sensorLocation: Point) {
  const points: Point[] = [];
  const GEODESY = createSphericalGeodesy(sensorLocation.reference as CoordinateReference);
  for (let i=0; i<6; ++i) {
...
export function createPentagon3D(sensorLocation: Point, minHeight: number, maxHeight: number) {
  const shape2D = createPentagon2D(sensorLocation);
  return createExtrudedShape(shape2D.reference,
    shape2D,
    minHeight,
    maxHeight
  )
