...
import {createPentagon3D} from "../controls/panocontroller/shapes/HexagonShape";
...
const regularColor = "rgba(43,196,206,1)";
const hoverColor = "rgba(178,255,82,1)";
const activeColor = "rgba(255,255,76, 0.25)";

const NormalShapeStyle: ShapeStyle= {
  stroke: {
    color: regularColor,
    width: 1
  },
...
const HoveredShapeStyle: ShapeStyle= {
  stroke: {
    color: hoverColor,
    width: 1
  },
...
const ActiveShapeStyle: ShapeStyle= {
  stroke: {
    color: activeColor,
    width: 1
  },
...
export class PanoramaFeaturePainterV2 extends FeaturePainter {
  ...
  private readonly _regularShapeStyle: ShapeStyle;
  private readonly _hoverShapeStyle: ShapeStyle;
  private readonly _activeShapeStyle: ShapeStyle;
  ...
  private _controllerPainter: boolean | undefined;
  ...
  constructor(options?: PanoramaFeaturePainterConstructorOptions) {
    ...
    const regularColor = "rgba(43,196,206,1)";
    const hoverColor = "rgba(178,255,82,1)";
    const activeColor = "rgba(255,255,76, 1)";

    const baseStyle3D = {
      mesh: createPanorama3DIcon()
    };

    this._activeShapeStyle = ActiveShapeStyle;
    this._regularShapeStyle = NormalShapeStyle;
    this._hoverShapeStyle = HoveredShapeStyle;
    ...
    this._controllerPainter = !!options?.controllerPainter;
  ...
  override paintBody(geoCanvas: GeoCanvas, feature: Feature, shape: Shape, layer: Layer, map: RIAMap,
    ...
    if (this._isOverview) {
      ...
      const newShape = createPentagon3D(sensorLocation, sensorLocation.z,sensorLocation.z + 0.05)
      if (this._activeFeatureLayer === layer && this._activeFeatureId === feature.id) {
        geoCanvas.drawShape(newShape, {...this._activeShapeStyle, occlusionMode: OcclusionMode.ALWAYS_VISIBLE});
      } else if (this._hoverFeatureId === feature.id) {
        geoCanvas.drawShape(newShape, {...this._hoverShapeStyle, occlusionMode: OcclusionMode.ALWAYS_VISIBLE});
      } else {
        geoCanvas.drawShape(newShape, {...this._regularShapeStyle, occlusionMode: OcclusionMode.ALWAYS_VISIBLE});
      }
      if (!this._controllerPainter) this.paintPanorama(geoCanvas, feature, shape);
    }
  }
  ...
  setActive(feature: Feature | null, layer?: FeatureLayer | null): void {
  ...
  override on(event: typeof ACTIVE_PANORAMA_CHANGED_EVENT | typeof HOVER_PANORAMA_CHANGED_EVENT | FeaturePainterEvents,
