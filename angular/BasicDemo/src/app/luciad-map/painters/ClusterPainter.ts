...
import {isCluster} from "@luciad/ria/view/feature/transformation/ClusteringTransformer";
...
export class ClusteringPainter extends BasicFeaturePainter {
    ...
    constructor(delegatePainter: FeaturePainter) {
        super();
        let pointStyle = this.getStyle(ShapeType.POINT, {hovered: false, selected: false});
        let pointStyleSelected = this.getStyle(ShapeType.POINT, {hovered: false, selected: true});


        this.setStyle( ShapeType.POINT,
            { hovered: false, selected: false
        }, DEFAULT_CLUSTER_ICON_STYLE);
        this.setStyle( ShapeType.POINT,
            { hovered: false, selected: true },
            DEFAULT_CLUSTER_ICON_SELECTED_STYLE);
  ...
  override paintBody(geoCanvas: GeoCanvas, feature: Feature, shape: Shape, layer: Layer, map: Map,
    ...
    paintClusterBody(geoCanvas: GeoCanvas, feature: Feature, shape: Shape, layer: Layer, map: Map,
                     paintState: PaintState): void {
        super.paintBody(geoCanvas, feature, shape, layer, map, paintState);
    }

  override paintLabel?(labelCanvas: LabelCanvas, feature: Feature, shape: Shape, layer: Layer, map: Map,
                paintState: PaintState): void {
        if (isCluster(feature)) {
            // Uncomment later
    ...
    protected paintClusterLabel(labelCanvas: LabelCanvas, feature: Feature, shape: Shape, layer: Layer, map: Map,
                                paintState: PaintState): void {
  ...
  override paintBorderBody(borderGeoCanvas: BorderGeoCanvas, feature: Feature, shape: Shape, layer: Layer, map: Map,
    ...
    paintClusterBorderBody(borderGeoCanvas: BorderGeoCanvas, feature: Feature, shape: Shape, layer: Layer, map: Map,
                           paintState: PaintState): void {
        //nothing by default
    }

  override paintBorderLabel(borderLabelCanvas: BorderLabelCanvas, feature: Feature, shape: Shape, layer: Layer, map: Map,
                     paintState: BorderPaintState): void {
        if (isCluster(feature)) {
    ...
    paintClusterBorderLabel(borderLabelCanvas: BorderLabelCanvas, feature: Feature, shape: Shape, layer: Layer, map: Map,
                            paintState: PaintState): void {
        //nothing by default
    }
