...
const defaultUrl = "https://demo.luciad.com/PortAIDemo/assets/panos/features.json";

@Component({
  selector: 'app-connect-panoramics',
  templateUrl: './connect-custom-panoramics.component.html',
  styleUrls: ['./connect-custom-panoramics.component.css']
})
export class ConnectCustomPanoramicsComponent {
  public url: string = defaultUrl;
  public label: string = "Custom Panorama";
  ...
  private addLayer() {
    ...
    if (this.url.trim().length>0 && this.label.trim().length>0){
      const layerInfo =  {
        layerType: UILayerTypes.CustomPanoramicLayer,
        model: {
          url: this.url.trim(),
          crs: this.crs
        },
