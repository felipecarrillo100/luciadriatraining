...
import {distance3D} from "./Vector3Util";
 ...
 * coordinates. Therefore it takes into account the distortion caused by the projection.
 */
function calculateMapUnitPerMeterRatioAtMapCenter(map: Map): number {
  ...
  const mapToModelTransformation = createTransformation(worldReference, map.reference.referenceType == ReferenceType.CARTESIAN ? map.reference : WGS84);
  const geodesy = map.reference.referenceType == ReferenceType.CARTESIAN ? createCartesianGeodesy(map.reference) : createEllipsoidalGeodesy(WGS84);
  ...
  try {
    ...
    if (distanceInMeters === 0.0) {
      ...
      const mapDistance = distance3D(mapLeftPoint, mapRightPoint);
