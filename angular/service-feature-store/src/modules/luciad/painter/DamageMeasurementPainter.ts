...
import { MeasurementPaintStyles } from '../controllers/ruler3d/measurement/Measurement';
...
export const PAINT_STYLES: MeasurementPaintStyles = {
  stationingStyles: [
    ...
    {
      stroke: {
        color: OCCLUDED_STATIONING_LINE_COLOR,
        width: 2,
        dash: [7, 3],
      },
      occlusionMode: OcclusionMode.OCCLUDED_ONLY,
...
/**
 * Feature Painter for the Luciad RIA Application Damage Boundary measurements
 */
export class DamageMeasurementPainter extends FeaturePainter {
  override paintBody(
    geoCanvas: GeoCanvas,
    feature: Feature,
    shape: Shape,
    ...
    if (!wrapper.expanded) {
      for (const style of COLLAPSED_STYLES) {
        ...
        geoCanvas.drawIcon(feature.shape, overritedStyle);
      }
    }
  }
