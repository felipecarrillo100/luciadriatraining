...
import { clamp, DEG2RAD, RAD2DEG } from '@pages/map/luciad-map/controllers/util/Math';
...
export class LookFromPanController extends Controller {
  ...
  override onGestureEvent(gestureEvent: GestureEvent): HandleEventResult {
    ...
    if (gestureEvent.type === GestureEventType.DRAG) {
      ...
      const maxDragStopTime =
        gestureEvent.inputType === 'touch' ? INERTIA_DRAG_STOP_TIME_TOUCH : INERTIA_DRAG_STOP_TIME_MOUSE;
      ...
      if (!dragStopped && speed > INERTIA_MIN_SPEED) {
        const panInertiaAnimation = new InertiaAnimation(
          ...
          viewPoint.x,
          viewPoint.y,
          speedX * INERTIA_SPEED_MULTIPLIER,
          speedY * INERTIA_SPEED_MULTIPLIER,
          this._panCamera,
        );
