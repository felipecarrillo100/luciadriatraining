...
import {ELayerId} from '../../../../../app/services/feature-layer-store.service';
...
export class PanoramaActions {
  ...
  constructor(map: WebGLMap, options?: PanoramaActionsConstructorOptions) {
    ...
    this._llhGeodesy = createEllipsoidalGeodesy(getReference(CRSEnum.CRS_84));
  ...
  moveToPanorama(feature: Feature, layer: FeatureLayer): void {
    if (layer.label !== ELayerId.panorama) return;
    ...
    const oldLayer = this._currentPanoFeatureLayer;
    this._currentPanoFeature = feature;
    this._currentPanoFeatureLayer = layer;
    const oldPainter = oldLayer?.painter as PanoramaFeaturePainter;
    const painter = layer.painter as PanoramaFeaturePainter;
    const fadeAnimFinished = (): void => {
      if (
        (oldLayer && this._currentPanoFeatureLayer && oldLayer.id !== this._currentPanoFeatureLayer.id) ||
        (oldLayer && !this._currentPanoFeatureLayer)
      ) {
        if (oldPainter?.setPanoOpacity) {
          oldPainter.setPanoOpacity(oldFeature, 0);
        }
      } else if (oldFeature && this._currentPanoFeature && oldFeature.id !== this._currentPanoFeature.id) {
        painter.setPanoOpacity && painter.setPanoOpacity(oldFeature, 0);
      }
    };

    const fadeInAnim = new FadePanoramaAnimation(painter, feature, 1, this._enterOptions.duration);

    AnimationManager.putAnimation(createFadePanoAnimationKey(feature), fadeInAnim, false)
      .then(fadeAnimFinished)
      .catch(fadeAnimFinished);
    ...
    if (feature.shape && feature.shape.focusPoint) {
      ...
      moveCameraToPoint(this._map, feature.shape.focusPoint, this._enterOptions.duration).then(() => {
        oldPainter?.setActive && oldPainter?.setActive(null);
        // @ts-ignore
        painter?.setActive && painter?.setActive(feature, layer);
  ...
  leavePanoramaMode(): void {
    if (this._currentPanoFeature && this._currentPanoFeatureLayer) {
      ...
      const fadeAnimation = new FadePanoramaAnimation(
        painter,
        this._currentPanoFeature,
        0,
        this._leaveOptions.duration,
      );
      const animFinished = (): void => {
        painter.setActive && painter.setActive(null);
      };
      AnimationManager.putAnimation(createFadePanoAnimationKey(this._currentPanoFeature), fadeAnimation, false)
        .then(animFinished)
        .catch(animFinished);
    }
    ...
    if (this._map.camera instanceof PerspectiveCamera) {
      ...
      const llhRef = getReference(CRSEnum.CRS_84);
  ...
  on(
    event: typeof START_MOVE_TO_PANORAMA_EVENT,
    callback: (feature: Feature | null, layer: FeatureLayer | null) => void,
  ): Handle;
  on(
    event: typeof END_MOVE_TO_PANORAMA_EVENT,
    callback: (feature: Feature | null, layer: FeatureLayer | null) => void,
  ): Handle;
  on(event: typeof LEFT_PANORAMA_MODE_EVENT, callback: () => void): Handle;
  on(event: typeof OPEN_SIDE_PANEL_ON_FEATURE_CLICK, callback: (feature: Feature, layer: FeatureLayer) => void): Handle;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  on(event: string, callback: (...args: any[]) => any): Handle {
    return this._eventedSupport.on(event, callback);
  }

  openSidePanel(feature: Feature, layer: FeatureLayer): void {
    this._eventedSupport.emit(OPEN_SIDE_PANEL_ON_FEATURE_CLICK, feature, layer);
  }
}
