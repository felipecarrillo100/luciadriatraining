...
export const OPEN_SIDE_PANEL_ON_FEATURE_CLICK = 'OpenSidePanelOnFeatureClick';
 ...
 * </code>
 */
export class PanoFeatureClickController extends Controller {
  private readonly _eventedSupport: EventedSupport;

  constructor() {
    super();
    this._eventedSupport = new EventedSupport([FEATURE_CLICKED, OPEN_SIDE_PANEL_ON_FEATURE_CLICK]);
  }

  override onGestureEvent(gestureEvent: GestureEvent): HandleEventResult {
    let pick;

    const openPanoramaOnFeatureClick = () => {
      pick ||= this.map.pickClosestObject(gestureEvent.viewPoint.x, gestureEvent.viewPoint.y, PICK_SENSITIVITY);

      if (pick?.layer instanceof FeatureLayer) {
    ...
    const openSidePanelOnFeatureClick = () => {
      pick ||= this.map.pickClosestObject(gestureEvent.viewPoint.x, gestureEvent.viewPoint.y, PICK_SENSITIVITY);

      if (pick?.layer instanceof FeatureLayer) {
        const pickedObject = pick.objects[0];
        this._eventedSupport.emit(OPEN_SIDE_PANEL_ON_FEATURE_CLICK, pickedObject, pick.layer, gestureEvent);
        return HandleEventResult.EVENT_HANDLED;
      }
    ...
    if (gestureEvent.inputType === 'mouse' && (gestureEvent.domEvent as MouseEvent).button === 0) {
      if (gestureEvent.type === GestureEventType.DOUBLE_CLICK) {
        openPanoramaOnFeatureClick();
      }

      if (gestureEvent.type === GestureEventType.SINGLE_CLICK_CONFIRMED) {
        openSidePanelOnFeatureClick();
      }
    }

    if (gestureEvent.inputType === 'touch') {
      if (gestureEvent.type === GestureEventType.DOUBLE_CLICK) {
        openPanoramaOnFeatureClick();
      }
  ...
  override on(
    event:
      ...
      | typeof OPEN_SIDE_PANEL_ON_FEATURE_CLICK
      | 'Invalidated'
      | 'Activated'
      | 'Deactivated',
    ...
    } else if (event === OPEN_SIDE_PANEL_ON_FEATURE_CLICK) {
      return this._eventedSupport.on(event, callback, context);
