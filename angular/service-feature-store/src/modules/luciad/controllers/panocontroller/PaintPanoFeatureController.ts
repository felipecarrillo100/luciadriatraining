...
import { createHexagon3D } from '../../painters/shapes/HexagonShape';
import { ShapeStyle } from '@luciad/ria/view/style/ShapeStyle';

interface FeaturePainterExtended extends FeaturePainter {
  _sensorHeight?: number;
  _hoverShapeStyle?: ShapeStyle;
  _regularShapeStyle?: ShapeStyle;
}
...
export class PaintPanoFeatureController extends Controller {
  ...
  private paintBody(geoCanvas: GeoCanvas, shape: Shape, paintState: PaintState, painter: FeaturePainterExtended): void {
    const sensorLocation = (shape as Point).copy();
    sensorLocation.translate3D(0, 0, -painter._sensorHeight);
    const newShape = createHexagon3D(sensorLocation, sensorLocation.z, sensorLocation.z + 0.1);
