...
import { rayRectangleIntersection, toPoint } from '../util/Vector3Util';
...
import {
  ...
  ROTATED_EVENT,
  STYLE_UPDATED_EVENT,
} from './geolocation/GeolocateHandleSupport';
...
export class BoxMoveController extends Controller {
  ...
  constructor(support: OrientedBoxEditingSupport, options?: BoxMoveControllerCreateOptions) {
    ...
    this._handleSupport.on(MOVED_EVENT, (translation: Vector3) => {
      this._editSupport.translate(translation);
    });
    this._handleSupport.on(ROTATED_EVENT, (absoluteRotation: number) => {
    ...
    this._handleSupport.on(ALTITUDE_CHANGED_EVENT, (translation: Vector3) => {
      this._editSupport.translate(translation);
    });
  }
  ...
  onDraw(geoCanvas: GeoCanvas) {
    this._editSupport.onDraw(geoCanvas);
