...
import { Axis } from '@luciad/ria/reference/Axis';
...
import { DEG2RAD, interpolate, RAD2DEG } from './Math';
import { LineType } from '@luciad/ria/geodesy/LineType';
import {CRSEnum} from '../../interfaces/CRS.enum';

const EPSILON = 1e-4;
const LLH_REFERENCE = getReference(CRSEnum.CRS_84);
const LLH_GEODESY = createEllipsoidalGeodesy(LLH_REFERENCE);

/**
 * Creates a shape that forms a horizontal arc arrow. (with arrowheads on both sides)
 */
export const createHorizontalArcArrow = (
  centerPoint: Point,
  ...
  const arrowShape = createExtrudedShape(
    centerPoint.reference,
    createArc(
      centerPoint.reference,
      centerPoint,
      radiusInMeters,
  ...
  const arrowHead1 = createExtrudedShape(
    centerPoint.reference,
  ...
  const arrowHeadPoints2 = [
    geodesy.interpolate(
      centerPointWithoutHeight,
      radiusInMeters - arrowHeadSizeMeters,
  ...
  const arrowHead2 = createExtrudedShape(
    centerPoint.reference,
 ...
 */
export const createHorizontalBarbedArcArrow = (
  centerPoint: Point,
  radiusInMeters: number,
  startAzimuthDegrees: number,
  arcSizeDegrees: number,
  ...
  return createShapeList(centerPoint.reference, [
    createPolygon(centerPoint.reference, [...outerLineCorners, ...innerLineCorners.reverse()]),
    endArrow,
    startArrow,
  ]);
};
...
/**
 * Creates a shape that forms a vertical line that acts as an arrow (with arrowheads on both sides)
 */
export const createVerticalLineArrow = (
  bottomPoint: Point,
  height: number,
  arrowHeadSizeMeters: number,
  geodesy: Geodesy = createEllipsoidalGeodesy(bottomPoint.reference),
): ShapeList => {
  ...
  const altitudeArrowHeadBasePoint1 = geodesy.interpolate(bottomPointWithoutHeight, arrowHeadSizeMeters, 270);
  const altitudeArrowHeadBasePoint2 = geodesy.interpolate(bottomPointWithoutHeight, arrowHeadSizeMeters, 90);
  const altitudeArrowHead1 = createPolygon(bottomPoint.reference, [
    ...
    createPoint(bottomPoint.reference, [
      bottomPointWithoutHeight.x,
      bottomPointWithoutHeight.y,
      bottomPoint.z - arrowHeadSizeMeters * 2,
    ]),
  ]);
  const altitudeArrowHead2 = createPolygon(bottomPoint.reference, [
    createPoint(bottomPoint.reference, [
      altitudeArrowHeadBasePoint1.x,
      altitudeArrowHeadBasePoint1.y,
      bottomPoint.z + height,
    ]),
    createPoint(bottomPoint.reference, [
      altitudeArrowHeadBasePoint2.x,
      altitudeArrowHeadBasePoint2.y,
      bottomPoint.z + height,
    ]),
    createPoint(bottomPoint.reference, [
      bottomPointWithoutHeight.x,
      bottomPointWithoutHeight.y,
      bottomPoint.z + height + arrowHeadSizeMeters * 2,
    ]),
  ]);
  return createShapeList(bottomPoint.reference, [verticalLine, altitudeArrowHead1, altitudeArrowHead2]);
};

export const createVerticalBarbedLineArrow = (
  bottomPoint: Point,
  height: number,
  azimuth: number,
  arrowHeadLengthInMeters: number,
  ...
  const bottomArrowHead = createVerticalBarbedArrowHead(
    leftBottomPoint,
    rightBottomPoint,
    arrowHeadLengthInMeters,
    arrowLineWidthInMeters,
  ...
  const topArrowHead = createVerticalBarbedArrowHead(
    leftTopPoint,
    rightTopPoint,
    arrowHeadLengthInMeters,
    arrowLineWidthInMeters,
  ...
  return createShapeList(bottomPoint.reference, [arrowLine, bottomArrowHead, topArrowHead]);
};

/**
 * Creates a shape that forms a horizontal line that acts as an arrow (with arrowheads on both sides)
 */
export const createHorizontalLineArrow = (
  centerPoint: Point,
 ...
 */
export const createHorizontalBarbedCrossArrow = (
  centerPoint: Point,
  radiusInMeters: number,
  azimuth: number,
  arrowHeadLengthInMeters: number,
  ...
  for (let i = 0; i < 4; i++) {
    const corner1 = LLH_GEODESY.interpolate(
      centerPoint,
      Math.sqrt(2 * arrowLineWidthInMeters * arrowLineWidthInMeters) / 2,
      azimuth + 45 - 90 * i,
    );
    ...
    arrowHeads.push(createHorizontalBarbedArrowHead(corner2, corner3, arrowHeadLengthInMeters, arrowLineWidthInMeters));
  }
  return createShapeList(centerPoint.reference, [
    createPolygon(centerPoint.reference, arrowLineCorners),
    ...arrowHeads,
  ]);
};
...
const createVerticalBarbedArrowHead = (
  ...
  const arrowOuterCornerRight = geodesy.interpolate(
    arrowInnerCornerRight,
    Math.sin(45 * DEG2RAD) * barbWidthInMeters,
    azimuth + 90,
  );
  ...
  const arrowOuterCornerLeft = geodesy.interpolate(
    arrowInnerCornerLeft,
    Math.sin(45 * DEG2RAD) * barbWidthInMeters,
    azimuth - 90,
  );
...
/**
 * Creates a shape that contains only the groundplane of an oriented box
 */
export const createGroundPlaneFromOrientedBox = (
  orientedBox: OrientedBox,
  transformation: Transformation,
  targetReference: CoordinateReference,
): Polygon => {
 ...
 */
export const createPointForOrientedBox = (
  cornerIndex: number,
  orientedBox: OrientedBox,
  transformation: Transformation,
): Point => {
 ...
 *                 orientedbox
 */
export const createStretchedOrientedBox = (
  orientedBox: OrientedBox,
  scaleX: number,
  scaleY: number,
  stretchZ: number,
...
export function createFacePolygons(box: OrientedBox) {
  ...
  return [
    createPolygon(box.reference as CoordinateReference, [cp[4], cp[6], cp[7], cp[5]]),
    createPolygon(box.reference as CoordinateReference, [cp[0], cp[1], cp[3], cp[2]]),
    createPolygon(box.reference as CoordinateReference, [cp[2], cp[3], cp[7], cp[6]]),
    createPolygon(box.reference as CoordinateReference, [cp[0], cp[1], cp[5], cp[4]]),
    createPolygon(box.reference as CoordinateReference, [cp[1], cp[3], cp[7], cp[5]]),
    createPolygon(box.reference as CoordinateReference, [cp[2], cp[6], cp[4], cp[0]]),
  ];
}
