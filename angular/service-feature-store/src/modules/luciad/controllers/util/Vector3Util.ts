...
export function invert(a: Vector3): Vector3 {
  return {
    x: -a.x,
    y: -a.y,
    z: -a.z,
...
// If Z is height, this function will return the distance between two points in 2D
...
/**
 * Returns the given point rotated with given angle (in degrees) around the given line.
 */
...
export function rayRectangleIntersection(
  ...
  const intersectionPoint = rayPlaneIntersection(rayOrigin, rayDirection, cross(edge1, edge2), rectangle.getPoint(0));
  if (intersectionPoint) {
    //check that the ray plane intersection is inside the rectangle
    const dotDir1 = scalar(edge1, sub(intersectionPoint, rectangle.getPoint(0)));
    const dotDir2 = scalar(edge2, sub(intersectionPoint, rectangle.getPoint(0)));
    if (0 <= dotDir1 && dotDir1 <= scalar(edge1, edge1) && 0 <= dotDir2 && dotDir2 <= scalar(edge2, edge2)) {
...
export const toPoint = (reference: CoordinateReference | null, vec: Vector3): Point =>
  createPoint(reference, [vec.x, vec.y, vec.z]);
