...
import { add, distance, projectPointOnLine, scale, toPoint } from '@pages/map/luciad-map/controllers/util/Vector3Util';
...
import { NavigationType } from '../gesture/GestureUtil';
...
export class AnchorSupport {
  ...
  constructor(map: Map, bounds: Bounds) {
    ...
    this._boundsCenter = map.reference.equals(bounds.reference)
      ? bounds.focusPoint
      : createTransformation(bounds.reference, map.reference).transform(bounds.focusPoint);
  ...
  computeAnchor(viewPoint: Point, navigationType: NavigationType) {
    ...
    if (anchor) {
      ...
      const stickyPoint = this.findStickyPoint(viewPoint) || this.findFallbackStickyPoint(navigationType);
  ...
  private findStickyPoint(viewPoint: Point, startRadius = 1): Vector3 | null {
    const coordinateCandidates = getSurroundingCoordinates(viewPoint, startRadius, this._map.viewSize);
    ...
    for (let i = 0; i < coordinateCandidates.length; i += delta) {
      ...
      const anchorCandidate = this.getAnchorAtViewPoint(toPoint(null, { x, y, z: 0 }));
      if (anchorCandidate) {
    ...
    if (startRadius < MAX_STICKY_RADIUS) {
      const radiusDelta = startRadius < 7 ? 1 : Math.max(1, Math.ceil(0.15 * startRadius));
  ...
  private findFallbackStickyPoint(navigationType: NavigationType) {
    ...
    if (this.isBoundsCenterInView()) {
      ...
      const distance = navigationType === NavigationType.ROTATION ? 1 : eyeToCenter / 4;
