...
const EPSG_3857 = getReference(CRSEnum.EPSG_3857);
 ...
 */
export const horizontalMovePointInteraction = (
  map: Map,
  viewPoint: Point,
  modelPoint: Point,
  options?: {
  ...
  const viewToMapTransformation = fixedHeight
    ? map.getViewToMapTransformation(LocationMode.ELLIPSOID, { heightOffset: modelPointWGS84.z })
  ...
  return (updatedViewPoint: Point): Point => {
    let newWgs84Point = mapToWgs84Transformation.transform(
      toPoint(WGS_84, add(offset, viewToMapTransformation.transform(updatedViewPoint))),
    );
    if (fixedHeight) {
 ...
 */
export const verticalMovePointInteraction = (
  map: Map,
  viewPoint: Point,
  modelPoint: Point,
): ((point: Point) => Point) => {
  ...
  return (updatedViewPoint: Point): Point => {
    ...
    if (touchedPoint) {
      ...
      throw new Error(
        'Updated view point should be touching the up-plane. ' +
 ...
 * which usually equates a single drag operation)
 * @param map the map to perform the operations on
 * @param viewPoint the original viewpoint at the start of the interaction (screen coordinates)
 * @param originalPoint the original point that needs to be moved (in any reference system)
 * @param planeNormal the normal (specified in EPSG_4978) of the plane that the 3D point can move on (ignored for EPSG_3857)
 * @returns function that returns updated point, given new values of the mouse location
 */
export function planarMovePointInteraction(
  map: Map,
  viewPoint: Point,
  originalPoint: Point,
  planeNormal?: Vector3, // Ignored for EPSG_3857
): (p: Point) => Point {
  // Handle EPSG_4978 (3D case)
  ...
  // Handle EPSG_3857 (2D case)
  if (map.reference.equals(EPSG_3857)) {
    // Ensure originalPoint is in EPSG_3857
    if (!EPSG_3857.equals(originalPoint.reference)) {
      originalPoint = createTransformation(originalPoint.reference!, EPSG_3857).transform(originalPoint);
    }

    // Use view-to-map transformation at z=0 (ground level)
    const viewToMapTransformation = map.getViewToMapTransformation(LocationMode.ELLIPSOID, { heightOffset: 0 });

    // Calculate initial offset between original point and starting view point in map space
    const startPoint = viewToMapTransformation.transform(viewPoint);
    const offset = sub(originalPoint, startPoint);

    const lastMovedPoint = originalPoint.copy();

    return (updatedViewPoint: Point): Point => {
      // Transform updated view point to map coordinates and apply offset
      const newMapPoint = add(offset, viewToMapTransformation.transform(updatedViewPoint));
      lastMovedPoint.move3DToCoordinates(newMapPoint.x, newMapPoint.y, 0); // Keep z=0 for 2D
      return lastMovedPoint;
    };
  }

  // Fallback for unsupported reference systems
  return () => originalPoint.copy();
}
 ...
 *                     center of rotation.
 */
export const horizontalRotateInteraction = (
  map: Map,
  viewPoint: Point,
  modelRotationPoint: Point,
  options?: { azimuthOffset?: number },
): ((point: Point) => number) => {
...
export function horizontalMouseRotateCheck(
  ...
  const rotateHandleBaseEllipse = createEllipse(handleCenter.reference, handleCenter, radius, radius, 0);
 ...
 */
export const directionalMovePointInteraction = (map: Map, startWorldPoint: Vector3, worldDirection: Vector3) => {
  ...
  return (viewPoint: Point): Vector3 => {
    ...
    const intersectionPoint = rayPlaneIntersection(camera.eye, pointingDir, pointingPlaneNormal, startWorldPoint);
    if (!intersectionPoint) {
      return startWorldPoint;
    } else {
      return projectPointOnLine(intersectionPoint, startWorldPoint, worldDirection);
    }
  };
};
...
export const closeToPointCheck = (
  map: Map,
  modelPoint: Point,
 ...
 */
export const closeToHorizontalPointCheck = (
  map: Map,
  point: Point,
  options: {
  ...
  return (viewPoint: Point): boolean => {
    ...
    if (touchedPoint) {
      if (options.maxWorldDistance) {
        ...
        return (
          distance(viewPoint, map.mapToViewTransformation.transform(toPoint(EPSG_4978, touchedPoint))) <
 ...
 */
export const closeToVerticalLineCheck = (
  map: Map,
  start: Point,
  end: Point,
  ...
  return (viewPoint: Point): boolean => {
    ...
    if (touchedPoint) {
      ...
      if (options.maxWorldDistance) {
        ...
        return (
          distance(viewPoint, map.mapToViewTransformation.transform(closestPointOnLine)) < options.maxPixelDistance!
        );
      }
    } else {
      return false;
 ...
 */
export const limitHorizontalMoveToBounds =
  (boundsLimitWGS84: Bounds) =>
  (wgs84Point: Point): Point => {
 ...
 */
function discretizeArc(ellipse: Ellipse, startAzimuth: number, sweepAngle: number, pointCount: number): Point[] {
  const points: Point[] = [];
