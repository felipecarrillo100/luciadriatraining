...
import FormatUtil from './Ruler3DController/FormatUtil';

import './RullerController.scss';
...
class RulerController extends Controller {
  private pvCartesian: unknown;
  private pvOptions: unknown;
  ...
  private onMeasurementUpdate: unknown;
  ...
  constructor(options?: {
    ...
    if (
      ![LineType.SHORTEST_DISTANCE, LineType.CONSTANT_BEARING, undefined].some((allowedLineType) => {
  ...
  public onGestureEvent(event: GestureEvent): number {
    switch (event.type) {
  ...
  public onDraw(geoCanvas: GeoCanvas) {
    ...
    for (let i = 0; i < this.pvPolygon.pointCount; ++i) {
      ...
      if (i > 0) {
        const line = ShapeFactory.createPolyline(this.pvPolyLine.reference, [
          this.pvPolyLine.getPoint(i - 1),
          this.pvPolyLine.getPoint(i),
        ]);
    ...
    if (this.rulerMode === RulerMode.AREA && this.pvPolyLine.pointCount > 2) {
      const lastLine = ShapeFactory.createPolyline(this.pvPolyLine.reference, [
        this.pvPolyLine.getPoint(this.pvPolyLine.pointCount - 1),
        this.pvPolyLine.getPoint(0),
      ]);
      ...
      if (this.rulerMode === RulerMode.DISTANCE) {
        ...
        if (this.pvPolyLine.pointCount === 2) {
          ...
          labelCanvas.drawLabel(
            tmpHtml,
            this.pvPolyLine.getPoint(this.pvPolyLine.pointCount - 1),
            labelTotalDistanceStyle,
          );
        }
        if (this.pvPolyLine.pointCount > 2) {
          const html = this.htmlLabel(this.pvText, 'rulerLabel');
          labelCanvas.drawLabelInPath(html, this.pvPolygon.focusPoint, {
          ...
          const lastPointHTML: string = this.htmlLabel(
            this.intermediateLabels[this.intermediateLabels.length - 1],
            'rulerPointLabel',
          );
  ...
  public onActivate(map: Map) {
    if (this.pvMap && this.pvMap !== map) {
      throw new Error(
        "A single instance of a AreaRulerController may not be active on multiple maps at the same time. Create new instances of AreaRulerController with 'new AreaRulerController(myOptions)'-syntax",
      );
    }
    this.init(map);
  ...
  private init(map: Map) {
    ...
    if (this.pvCartesian) {
      ...
      this.pvShapeReference = ReferenceProvider.getReference(CRSEnum.CRS_84);
  ...
  private createModelPoint(viewPosition: number[]) {
    let vwTrans;
    let worldPoint;
    let viewPoint;
    let modelPoint;
    ...
    try {
      if (this.pvMap) {
        ...
        viewPoint = ShapeFactory.createPoint(null, viewPosition);
  ...
  private withinSlack(event: { viewPosition: [number, number] }) {
    if (!this.pvWait) {
      return false;
    }
  ...
  private onDrag(event: { viewPosition: [number, number] }) {
    if (this.pvWait) {
  ...
  private onDragEnd(event: { viewPosition: [number, number] }) {
    if (this.withinSlack(event)) {
      return this.onClick(event);
    }
    this.letGo();
  ...
  private onMove(event: { viewPosition: [number, number] }) {
  ...
  private updateHover(event: { viewPosition: [number, number] }) {
    const mp = this.createModelPoint(event.viewPosition);
    if (!mp) {
      // if no point can be created (e.g. because outside bounds), just ignore the input
      return HandleEventResult.EVENT_HANDLED;
    }
  ...
  private onClickConfirmed(event: { viewPosition: [number, number] }) {
    let mp;
  ...
  private onDown(event: { viewPosition: [number, number] }) {
  ...
  private onClick(event: { viewPosition: [number, number] }) {
