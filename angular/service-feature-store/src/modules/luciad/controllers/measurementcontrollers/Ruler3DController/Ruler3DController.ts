...
interface Ruler3DControllerOptions {
  enabled?: boolean;
  measureType?: Ruler3DControllerTypes;
}
...
class Ruler3DController extends Controller {
  ...
  private _measureOptions: NonNullable<unknown>;
  ...
  constructor(optionsIn?: Ruler3DControllerOptions) {
    ...
    this._eventSupport = new EventedSupport(
      [
        Ruler3DController.MEASUREMENT_CHANGED,
        Ruler3DController.MEASUREMENT_DEACTIVATED,
        Ruler3DController.ENABLED_CHANGE_EVENT,
      ],
      true,
    );
    ...
    this.setMeasureOptions(
      MEASURE_TYPE.MEASURE_TYPE_PARAM,
      options[MEASURE_TYPE.MEASURE_TYPE_PARAM] || Ruler3DControllerTypes.MEASURE_TYPE_DISTANCE,
    );
  ...
  get segments(): unknown[] {
    return this._measurement.segments;
  }

  get totals(): { area: number; length: number } {
  ...
  public setMode(name: string) {
    this.setMeasureOptions(Ruler3DController.MEASURE_TYPE_PARAM, name as Ruler3DControllerTypes);
  ...
  private onDoubleClick = function () {
  ...
  private init(map: Map) {
    ...
    const shapeReference = ReferenceProvider.getReference(CRSEnum.EPSG_4326); // 3D WGS:84
