...
import {
  ...
  SinglePointDeleteHandle,
} from '@luciad/ria/view/editor/handles/PointListDeleteHandle.js';
...
class CustomSinglePointDeleteHandle extends SinglePointDeleteHandle {
  override shouldActivate(gestureEvent: GestureEvent, context: EditContext): boolean {
    return (
      gestureEvent.type === GestureEventType.SINGLE_CLICK_UP &&
  ...
  override shouldProcess(gestureEvent: GestureEvent): boolean {
    return gestureEvent.type === GestureEventType.SINGLE_CLICK_UP && isRightMouseButtonDown(gestureEvent);
  }
}
