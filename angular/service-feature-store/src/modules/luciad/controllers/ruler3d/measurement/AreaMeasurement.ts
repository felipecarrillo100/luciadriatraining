...
import { createPolygon, createPolyline } from '@luciad/ria/shape/ShapeFactory';
...
import { drawAreaLabel, drawDistanceLabel } from './MeasurementUtil';
...
export class AreaMeasurement extends Measurement<AreaMeasurementSegment, AreaMeasurementInfo> {
  constructor() {
    super(EAnnotationsTypes.Area);
  }
  ...
  recomputeSegment(segment: AreaMeasurementSegment): void {
    ...
    const { distance, shape, area } = calculateSegmentProperties(this.points[0], segment.p1, segment.p2);
  ...
  paintBody(geoCanvas: GeoCanvas, { mainLineStyles, areaStyles, pointStyles }: MeasurementPaintStyles): void {
    for (const { line, shape } of this.segments) {
      ...
      if (shape) {
        for (const style of areaStyles) {
          geoCanvas.drawShape(shape.copy(), style);
        }
      }
    }
  ...
  paintLabel(labelCanvas: LabelCanvas, styles: MeasurementPaintStyles): void {
    ...
    if (p0) {
      drawAreaLabel(labelCanvas, p0.copy(), styles.helperLabelHtmlStyle, this.totalInfo.surface);
    }

    for (const { distance, line } of this.segments) {
      drawDistanceLabel(labelCanvas, line.copy(), styles.mainLabelHtmlStyle, distance);
    }
    const { closingSegment } = this.totalInfo;
    if (closingSegment) {
      drawDistanceLabel(labelCanvas, closingSegment.line.copy(), styles.mainLabelHtmlStyle, closingSegment.distance);
    }
  }
  ...
  getFormattedTotalInfo() {
    return [
      {
        label: 'Surface',
        value: FormatUtil.area(this.totalInfo.surface),
      },
      {
        label: 'Perimeter',
        value: FormatUtil.distance(this.totalInfo.perimeter),
      },
    ];
  }
}
