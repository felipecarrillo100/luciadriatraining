...
export const MEASUREMENTS_MODEL_REFERENCE = getReference(CRSEnum.CRS_84);
export const GEODESIC_GEODESY = createEllipsoidalGeodesy(getReference(CRSEnum.CRS_84));
export const CARTESIAN_GEODESY = createCartesianGeodesy(getReference(CRSEnum.EPSG_4978));

export interface MeasurementPaintStyles {
  pointStyles: IconStyle[];
  mainLineStyles: ShapeStyle[];
  helperLineStyles: ShapeStyle[];
  stationingStyles: ShapeStyle[];
...
export abstract class Measurement<S extends MeasurementSegment = MeasurementSegment, T = NonNullable<unknown>> {
  private readonly _eventedSupport: EventedSupport;
  private readonly _type: EAnnotationsTypes;
  private _points: Point[] = [];
  private _segments: S[] = [];
  private _totalInfo: T;

  protected constructor(type: EAnnotationsTypes) {
