 ...
 */
import { createEllipse, createPolygon } from '@luciad/ria/shape/ShapeFactory';
...
export const horizontalMovePointInteraction = (
  ...
  viewPoint: Point,
  modelPoint: Point,
  options?: { fixedHeight?: boolean; restrictionFunction?: (point: Point) => Point },
): ((point: Point) => Point) => {
...
export const closeToHorizontalPointCheck = (
  map: Map,
  point: Point,
  options: { maxPixelDistance?: number; maxWorldDistance?: number },
): ((point: Point) => boolean) => {
...
export const createTranslationForOrientedBox = (
  map: Map,
  eventViewPoint: Point,
  modelPoint: Point,
  options?: { fixedHeight?: boolean; restrictionFunction?: (point: Point) => Point },
): ((point: Point) => Point) => {
  const pointerHeight = options?.fixedHeight !== false;
  const restrictionFunction = options?.restrictionFunction || null;
  ...
  const viewToWorldMapTransformation = pointerHeight
    ? map.getViewToMapTransformation(LocationMode.ELLIPSOID, { heightOffset: modelPointWGS84.z })
    : map.getViewToMapTransformation(LocationMode.TERRAIN);

  const modelPointInMapRef = createTransformation(modelPoint.reference, map.reference).transform(modelPoint);
  const initPoint = viewToWorldMapTransformation.transform(eventViewPoint);
  const offset = sub(modelPointInMapRef, initPoint);

  return (translatedPoint: Point): Point => {
    let translatedWgs84Point = mapToWgs84Transformation.transform(
      toPoint(map.reference, add(offset, viewToWorldMapTransformation.transform(translatedPoint))),
    );
    if (pointerHeight) {
      translatedWgs84Point.z = modelPointHeight;
    }
    if (restrictionFunction) {
      translatedWgs84Point = restrictionFunction(translatedWgs84Point);
    }
    return translatedWgs84Point;
  };
};
