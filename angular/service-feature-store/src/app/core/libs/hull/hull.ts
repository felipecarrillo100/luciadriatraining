...
import { intersect } from '@core/libs/hull/intersect';
import { formatUtilFromXY, formatUtilToXy } from '@core/libs/hull/format';
import { convexHull } from '@core/libs/hull/convex';
import { grid } from '@core/libs/hull/grid';
...
export function hull(pointset, concavity, format) {
  ...
  const points = _filterDuplicates(_sortByX(formatUtilToXy(pointset, format)));

  if (points.length < 4) {
    const concave = points.concat([points[0]]);
    return format ? formatUtilFromXY(concave, format) : concave;
  }
